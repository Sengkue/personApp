    ...
    <v-dialog v-model="dialog" persistent max-width="600px" class="pa-5">
      <v-card>
        <v-card-title class="d-flex justify-space-between">
          <span>ລາຍລະອຽດຂອງແຜນການ ແລະ ເປົ່າໝາຍ</span>
        ...
        <v-row class="pa-0 ma-0">
          ...
          <v-col cols="12" sm="8">
            <v-row>
              ...
              <v-col cols="5">ເວລາເລີ່ມຕົ້ນ:</v-col>
              <v-col cols="6">{{ formatDateTime(item?.startDate) }}</v-col>
              <v-col cols="5">ເວລາສິ່ນສຸດ:</v-col>
              <v-col cols="6">{{ formatDateTime(item?.endDate) }}</v-col>
              ...
              <v-col :cols="`${item?.problem?.length > 25 ? 12 : 6}`">{{
