    ...
    >
      <div class="d-flex justify-space-between align-center">
        <div class="py-2 text-caption text-sm-body-2 text-md-body-1 text-lg-h6">
          <h3 style="color: #e0f7fa">ລາຍຮັບ</h3>
      ...
      <div class="d-flex">
        <v-icon v-ripple color="white" @click="Income()">{{
          `mdi-arrow-${InComeStatus === 1 ? "down" : "up"}-circle-outline`
        }}</v-icon>
        <div>{{ getStatusText(InComeStatus) }}</div>
      </div>
    </v-card>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      moneyData: [],
      InComeStatus: 1, // Default to current date
    };
  },
  ...
  methods: {
    Income() {
      ...
      this.InComeStatus = this.InComeStatus % 4 + 1;
    },

    getMoneyData() {
      this.$axios.get("/moneyInCome.json").then((res) => {
