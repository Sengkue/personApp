...
export default {
  ...
  computed: {
    getItem() {
      return this.$store.state.code.type?.codingTypeData?.map(
        (item, index) => ({
          ...item,
          index: index + 1,
        })
  ...
  async mounted() {
    this.loading = true;
    await this.$store.dispatch("code/type/selectAll");
    this.loading = false;
    this.allUpdate();
  },
  methods: {
    ...
    async deleteItem() {
      this.loading = true;
      await this.$store.dispatch("code/type/deleteItem", this.deleteId);
