  ...
  <v-container style="max-width: 500px">
    ...
    >
      <template v-slot:append>
        <v-fade-transition>
          <v-icon v-if="planningTodoList?.task" @click="create">
            mdi-plus-circle
          </v-icon>
        </v-fade-transition>
      </template>
...
export default {
  ...
  computed: {
    tasks() {
      return this.$store.state.planningTodo.planningTodoData?.filter(
  ...
  methods: {
    ...
    create() {
      if (!this.planningTodoList.task) {
        this.$toast.error("ກະລຸນາປ້ອນຂໍ້ມູນກ່ອນ!");
        return;
      }
      this.planningTodoList.planId = this.$route.params?.id;
      this.$store.dispatch("planningTodo/addplanningTodo", this.planningTodoList);
      this.planningTodoList.task = null;
    },
    // ____________________________ update status __________________________
    async updateTaskStatus(item) {
      await this.$store.dispatch("planningTodo/updateplanningTodo", item);
    ...
    async updatTask() {
      this.loading = true;
      await this.$store.dispatch("planningTodo/updateTask", this.updateTask);
      this.loading = false;
      this.dialog = false;
    },
    ...
    async deleteTask() {
      this.loading = true;
      await this.$store.dispatch("planningTodo/deleteItem", this.updateTask.id);
      this.loading = false;
