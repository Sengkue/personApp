  ...
  <div>
    <v-row class="pa-1 ma-0">
      <v-app-bar v-if="$vuetify.breakpoint.xs" fixed app>
        ...
        <v-spacer />
        <v-toolbar-title class="ml-n10">Idea & ແນວຄວາມຄິດ</v-toolbar-title>
        <v-spacer />
        <div></div>
      </v-app-bar>
            ...
            <template #[`item.actions`]="{ item }">
              <div class="d-flex">
                <div class="d-flex flex-column">
                  <managesIdeaDelete :item="item?.id" class="mb-3" />
                  <managesIdeaView :item="item" />
                </div>
                <div class="d-flex flex-column">
                  <managesIdeaEdit :item="item" class="mb-3 ml-3" />
...
export default {
  ...
  computed: {
    getItem() {
      return this.$store.state.idea?.ideaData
  ...
  async mounted() {
    this.loading = true;
    await this.$store.dispatch("idea/selectAll");
    this.loading = false;
  },
