  ...
  <div>
    <v-row class="pa-1 ma-0">
      ...
      <v-btn class="pa-0 ma-0" icon @click="$router.push('/')">
        ...
        <v-spacer />
        <v-toolbar-title class="ml-n10">ຈັດການຂໍ້ມູນລາຍຈ່າຍ</v-toolbar-title>
        <v-spacer />
        <div></div>
      </v-app-bar>
      ...
      <v-col v-if="$vuetify.breakpoint.xs" cols="12" sm="12" class="pa-0 ma-0">
        <moneyOutComeMobileScreen :filteredItems="getItem" :search="search" />
      </v-col>
        ...
        <v-card>
          ...
          >
            ...
            <template #[`item.actions`]="{ item }">
              <div class="d-flex">
                <moneyOutComeDelete :item="item?.id" />

                <moneyOutComeEdit :item="item" />

                <moneyOutComeView :item="item" />
              </div>
...
export default {
  ...
  computed: {
    getItem() {
      return this.$store.state.moneyOutCome.moneyOutComeData?.map(
        (item, index) => ({
          ...item,
