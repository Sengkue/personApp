  ...
  <div>
    <v-row class="pa-1 ma-0">
      ...
      <v-btn class="pa-0 ma-0" icon @click="$router.push('/')">
        ...
        <v-spacer />
        <v-toolbar-title class="ml-n10">ຈັດການຂໍ້ມູນລາຍຮັບ</v-toolbar-title>
        <v-spacer />
        <div></div>
      </v-app-bar>
      ...
      <v-col v-if="$vuetify.breakpoint.xs" cols="12" sm="12" class="pa-0 ma-0">
        <moneyInComeMobileScreen :filteredItems="getItem" :search="search" />
      </v-col>
        ...
        <v-card>
          ...
          >
            ...
            <template #[`item.actions`]="{ item }">
              <div class="d-flex">
                <moneyInComeDelete :item="item.id" />

                <moneyInComeEdit :item="item" />
...
export default {
  ...
  computed: {
    getItem() {
      return this.$store.state.moneyInCome?.moneyInComeData?.map(
        (item, index) => ({
          ...item,
