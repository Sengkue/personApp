  ...
  <div>
    <v-row class="pa-1 ma-0">
      <v-app-bar v-if="$vuetify.breakpoint.xs" fixed app>
        ...
        <v-spacer />
        <v-toolbar-title class="ml-n10">ວາງແຜນການ</v-toolbar-title>
        <v-spacer />
        <div></div>
      </v-app-bar>
...
export default {
  ...
  computed: {
    getItem() {
      return this.$store.state.planGroup.planData
  ...
  async mounted() {
    this.loading = true;
    await this.$store.dispatch("planGroup/selectAll");
    this.loading = false;
  },
