<template>
  <div>
    <v-app-bar v-if="$vuetify.breakpoint.xs" fixed app>
      ...
      <v-spacer />
      <v-toolbar-title class="ml-n10">ວິດີໂອ</v-toolbar-title>
      <v-spacer />
      <div></div>
    </v-app-bar>
    ...
    <v-card class="rounded-0" elevation="0">
      ...
      >
        <v-col cols="12" class="my-1 py-0">
          ...
          >
            ...
            <div class="d-flex justify-center align-center">
              ...
              <h2></h2>
              <div v-if="answer?.indexChild" class="animate-charcter py-5">
                {{ answer?.index }} Videos {{ answer?.indexChild }}
              </div>
              <div v-else class="animate-charcter py-5">
                Videos {{ answer?.index }}
              </div>
              <v-icon>mdi-arrow-down-thin</v-icon>
            </div>
        ...
        <v-col cols="12" sm="11" md="11" lg="11" xl="11">
          <div>
            <div
              v-html="makeLinksClickable(sanitizeHTML(answer?.v_description))"
            ></div>
            <div>{{ answer?.v_image }}</div>
            <div>{{ answer?.v_microphone }}</div>
        ...
        <v-col class="mt-1" cols="12">
          <v-card elevation="0" class="d-flex justify-start">
            <VideosVideoDelete :item="answer?.id" />
            <VideosVideoEdit :item="answer" />
          </v-card>
        </v-col>
      </v-row>
    </v-card>
...
export default {
  ...
  getPostAnswer() {
    ...
    const videosData = this.$store.state.videos.videosData;
    if (typeId) {
      let filteredVideos = videosData.filter(video => video.typeId === typeId);
       let indexChild = 1;
  ...
  mounted() {
    this.$store.dispatch("videos/selectAll");
  },
  methods: {
    DeleteItem() {
