<template>
  <div >
    <v-app-bar v-if="$vuetify.breakpoint.xs" fixed app>
      ...
      <v-spacer />
      <v-toolbar-title class="ml-n10">ປະເພດວິດີໂອ</v-toolbar-title>
      <v-spacer />
      <div></div>
    </v-app-bar>
    ...
    <v-card class="pa-0 ma-0 rounded-0">
      <div class="d-flex align-center pl-2 mb-2 pt-2">
        <v-text-field
          v-model="search"
          label="Search"
          solo-inverted
      ...
      <v-card-text
        class="pa-0 ma-0"
        style="position: relative; height: calc(100vh - 17vh); overflow: auto"
      >
        ...
        >
          <v-card>
            <v-form ref="form" v-model="valid">
              ...
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="error" text @click="dialogUpdate = false">
                  ປິດ
                </v-btn>
                <v-btn
                  color="blue darken-1"
...
export default {
  ...
  computed: {
    getItem() {
      return this.$store.state.videos.type?.videotypeData?.map(
        (item, index) => ({
          ...item,
  ...
  async mounted() {
    this.loading = true;
    await this.$store.dispatch("videos/type/selectAll");
    this.loading = false;
    this.allUpdate();
  },
  methods: {
    ...
    async deleteItem() {
      this.loading = true;
      await this.$store.dispatch("videos/type/deleteItem", this.deleteId);
    ...
    selectItem(item) {
      this.$router.push(`/videos/${item.id}`);
      this.dialog = false;
    },
