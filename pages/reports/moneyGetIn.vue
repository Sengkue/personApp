<template>
  <div class="pa-1">
    <v-app-bar v-if="$vuetify.breakpoint.xs" fixed app>
      ...
      <v-toolbar-title class="ml-n10">ລາຍງານເງິນທີຢືມມາ</v-toolbar-title>
      <v-spacer />
      <div></div>
    </v-app-bar>
    ...
    <v-row v-if="$vuetify.breakpoint.xs" justify="center" class="pa-0 ma-0">
      <ReportGetInMobileScreen :filteredItems="filteredItems" />
    </v-row>
...
export default {
  ...
  computed: {
    filteredItems() {
      ...
      return this.$store.state.moneyGetIn?.moneyGetInData
